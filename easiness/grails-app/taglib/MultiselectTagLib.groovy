 ...
 *
 * Examples:
 * <g:multiselect multiple="multiple" optionKey="id" from="${theList}"
 *       name="selectListName"></g:multiselect>
 *
 */
...
class MultiselectTagLib {
   ...
   def multiselect = { attrs ->
    ...
    if (noSelection) {
        renderNoSelectionOption(noSelection.key, noSelection.value, value) writer.println()
    }
    ...
    if (from) {
        from.eachWithIndex { el,i ->
           ...
           if(keys) {
              ...
              } else if(el !=null && optionKey == 'id' && grailsApplication.getArtefact(DomainClassArtefactHandler.TYPE,
                                                                      el.getClass().name)) {
                 keyValue = el.ident()
              } else {
