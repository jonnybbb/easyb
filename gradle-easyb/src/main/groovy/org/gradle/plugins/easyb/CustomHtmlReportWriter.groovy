...
class CustomHtmlReportWriter implements ReportWriter {
    ...
    CustomHtmlReportWriter(String location) {
    ...
    private writeResourceToDisk(String resourceLocation, resourceName) {
        InputStream inputStream = this.
                class.getClassLoader().getResourceAsStream("${resourceLocation}/${resourceName}");
        if (inputStream != null) {
            FileOutputStream resourceWriter = new FileOutputStream("${locationDir}${File.separator}${resourceName}")
            inputStream.eachByte {
                singleByte -> resourceWriter.write(singleByte)
            }
